...
def train_one_epoch_temporal(
    ...
    for batch in tqdm(loader, desc="train-temporal", leave=False):
...
def evaluate_temporal_loader(
    ...
    with torch.no_grad():
        for batch in tqdm(loader, desc="eval-temporal", leave=False):
            videos = batch["video"].to(device, non_blocking=True)
            labels = batch["label"].to(device, non_blocking=True)
            lengths = batch["length"].to(device, non_blocking=True)
...
def fit_temporal_model(
    ...
    for epoch in range(1, epochs + 1):
        train_loss = train_one_epoch_temporal(
            model=model,
            loader=train_loader,
            criterion=criterion,
            optimizer=optimizer,
        ...
        val_metrics = evaluate_temporal_loader(
            model=model,
            loader=val_loader,
            criterion=criterion,
            device=device,
        ...
        if patience > 0 and stale_epochs >= patience:
            print(
                f"[INFO] temporal early stopping at epoch={epoch} "
...
def load_temporal_model_from_checkpoint(
    ...
    model = build_temporal_model(model_cfg=model_cfg, num_classes=num_classes)
